...
if ($cont == 0) {
	...
	$int1 = mysql_query("INSERT INTO VotoRespuesta (id_respuesta, votante, status) VALUES ('$id', '$votante', 1)",$link);
	if ($act1 && $int1) {
	...
	if ($status == 1) {
		...
		$upd = mysql_query("UPDATE VotoRespuesta SET status = 0 WHERE id_respuesta = '$id' AND votante = '$votante'", $link);
		if ($act2 && $upd) {
		...
		$upd2 = mysql_query("UPDATE VotoRespuesta SET status = 1 WHERE id_respuesta = '$id' AND votante = '$votante'", $link);
		if ($act3 && $upd2) {
